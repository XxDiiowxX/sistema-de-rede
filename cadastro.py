...
# LINK ATUALIZADO (O link que está no seu print atual)
...
# DASHBOARD LÍDER
    ...
    with aba2: st.dataframe(df_e[['nome', 'celular', 'whatsapp_validado', 'Tamanho da Equipe', 'tag_admin']], use_container_width=True)
    with aba3:
        alvo = st.selectbox("Membro:", df_e['nome'].tolist())
        t = st.text_input("Nova Tag:")
...
# ADMIN MASTER (CORRIGIDO COM AS TAGS)
elif st.session_state.perfil_acesso == "admin":
    st.title("👑 Admin Master")
    aba1, aba2, aba3, aba4 = st.tabs(["🏆 Global", "🌳 Árvore", "🏷️ Gestão de Tags", "🔑 Acessos"])
    
    with aba1: st.dataframe(df_rede, use_container_width=True)
    
    with aba2:
        for r in df_rede[df_rede['patrocinador'].isin(['nenhum', ''])]['nome'].tolist():
            desenhar_arvore(r, df_rede)
            
    with aba3:
        st.subheader("🏷️ Editar Tags de Qualquer Usuário")
        user_alvo_tag = st.selectbox("Selecione o Usuário:", df_rede['nome'].tolist(), key="admin_tag_select")
        nova_tag_admin = st.text_input("Nova Tag:", key="admin_tag_input")
        if st.button("Atualizar Tag Global"):
            cursor = conexao.cursor()
            cursor.execute("UPDATE usuarios SET tag_admin=? WHERE nome=?", (nova_tag_admin, user_alvo_tag))
            conexao.commit()
            st.success(f"Tag de {user_alvo_tag} atualizada!")
            st.rerun()

    with aba4:
        alvo = st.selectbox("Líder:", df_rede['nome'].tolist())
        sen = st.text_input("Senha:")
        lib = st.checkbox("Liberar Login")
