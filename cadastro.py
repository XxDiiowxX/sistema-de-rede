...
# LINK OFICIAL DO SEU APP (Para os links de indicação e validação)
URL_APP = "https://sistema-de-rede-mx7hwj9b34e6aeayiyv4pf.streamlit.app"
...
def conectar_banco():
    ...
    ''')
    # Garantir que colunas novas existam em bancos antigos
...
def enviar_mensagem_whatsapp(nome, celular):
    ...
    link_validacao = f"{URL_APP}/?validar={celular}"
        ...
        if st.form_submit_button("Cadastrar e Gerar Link", use_container_width=True):
            ...
            if n and len(c_clean) == 11:
                try:
                    ...
                    st.code(f"{URL_APP}/?ref={n}")
                    st.warning("Confirme o link enviado no seu WhatsApp!")
                except: st.error("Nome já existe.")
            else: st.error("Dados inválidos (WhatsApp precisa de 11 dígitos).")

    with st.expander("🔗 Recuperar meu link"):
        b = st.text_input("Seu nome cadastrado").lower().strip()
        if st.button("Buscar"):
            if b in nomes_cadastrados: st.code(f"{URL_APP}/?ref={b}")
...
# DASHBOARD LÍDER
    ...
    st.info(f"Seu Link: {URL_APP}/?ref={u}")
...
# ADMIN MASTER
    ...
    aba1, aba2, aba3 = st.tabs(["🏆 Global", "🌳 Árvore", "🔑 Acessos"])
